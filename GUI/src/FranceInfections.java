...
public class FranceInfections extends Thread {
    ...
    public FranceInfections() {
        start();

        JOptionPane.showMessageDialog(null, "THE VIRUS APPEARED IN FRANCE!");
    }


    @Override
    public void run() {
        while (infections <= 67_000_000) {
            try {
                Thread.sleep(1000);
            ...
            if (infections > 67_000_000) {
                infections = 67_000_000;
                return;
    ...
    public static void divideMultiple(int multiple) {
        FranceInfections.multiple = FranceInfections.multiple / multiple;
    }
