...
public class NorwayInfections extends Thread {
    ...
    public NorwayInfections() {
        start();

        JOptionPane.showMessageDialog(null, "THE VIRUS APPEARED IN NORWAY!");
    }


    @Override
    public void run() {
        while (infections <= 5_300_000) {
            try {
                Thread.sleep(1000);
            ...
            if (infections > 5_300_000) {
                infections = 5_300_000;
                return;
    ...
    public static void divideMultiple(int multiple) {
        NorwayInfections.multiple = NorwayInfections.multiple / multiple;
    }
