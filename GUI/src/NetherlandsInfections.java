...
public class NetherlandsInfections extends Thread {
    ...
    public NetherlandsInfections() {
        start();

        JOptionPane.showMessageDialog(null, "THE VIRUS APPEARED IN NETHERLANDS!");
    }


    @Override
    public void run() {
        while (infections <= 17_700_000) {
            try {
                Thread.sleep(1000);
            ...
            if (infections > 17_700_000) {
                infections = 17_700_000;
                return;
    ...
    public static void divideMultiple(int multiple) {
        NetherlandsInfections.multiple = NetherlandsInfections.multiple / multiple;
    }
