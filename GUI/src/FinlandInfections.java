...
public class FinlandInfections extends Thread {
    ...
    public FinlandInfections() {
        start();

        JOptionPane.showMessageDialog(null, "THE VIRUS APPEARED IN FINLAND!");
    }


    @Override
    public void run() {
        while (infections <= 5_500_000) {
            try {
                Thread.sleep(1000);
            ...
            if (infections > 5_500_000) {
                infections = 5_500_000;
                return;
    ...
    public static void divideMultiple(int multiple) {
        FinlandInfections.multiple = FinlandInfections.multiple / multiple;
    }
